...
//                   {course.pdfUrl && (
//                     <a href={`https://lms-server-915k.onrender.com${course.pdfUrl}`} target="_blank" rel="noopener noreferrer" style={{ color: '#388bff' }}>
//                       View PDF
//                     </a>
//                   )}
//                 </li>
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    if (active === "Assignments") {
      ...
      if (!user || !user._id) {
        setLogError("User not found. Please log in again.")
        return
      }
      setLogLoading(true)
      setLogError("")
  ...
  try {
    ...
    // Adjust based on your submitAssignment signature, e.g., adding a second argument
    await submitAssignment(assignmentId, formData); // Example: Added assignmentId as second argument
    setSubmissionMsg('Assignment submitted!');
    setSubmissionForm(prev => ({ ...prev, [assignmentId]: { answerText: '', file: null } }));
  } catch (err: any) {
    setSubmissionError(err.message);
...
ächter
          ...
          <div className="mb-8">
            <h4 className="text-lg font-bold mb-2" style={{ color: '#388bff' }}>Assignments</h4>
            {assnLoading && <div>Loading assignments...</div>}
            {assnError && <div className="text-red-500">{assnError}</div>}
            {!assnLoading && !assnError && assignments.length === 0 && (
              <div className="text-center py-8" style={{ color: '#b0c4de' }}>
            ...
            {!user || !user._id ? (
              ...
              <>
                <CodePlayground 
                  ...
                  onCodeSubmit={handleCodeSubmit}
                />
                {logLoading && <div>Loading playground logs...</div>}
                {logError && <div className="text-red-500">{logError}</div>}
                {!logLoading && !logError && logs.length === 0 && (
                  <div className="text-center py-8" style={{ color: '#b0c4de' }}>
                    No playground logs available.
                  </div>
                )}
                {!logLoading && !logError && logs.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-lg font-bold mb-2" style={{ color: '#388bff' }}>Playground Logs</h4>
                    <ul className="space-y-2">
                      {logs.map((log) => (
                        <li key={log._id} className="p-2 rounded shadow" style={{ backgroundColor: '#23293b', color: '#fff' }}>
                          <p>Code: {log.code}</p>
                          <p>Language: {log.language}</p>
